                        ...
                        <?php foreach (\wsb\App::$app->getProperty('languages') as $k => $lang): ?>
                            <div class="tab-pane fade <?php if ($lang['base']) echo 'active show' ?>" id="<?= $k ?>">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="category_desc[<?= $lang['id'] ?>][title]" class="form-control" id="title" placeholder="Заголовок" value="<?= h($category[$lang['id']]['title']) ?>" required2>
                                </div>

                                <div class="form-group">
                                    <label for="description">Короткий опис</label>
                                    <input type="text" name="category_desc[<?= $lang['id'] ?>][description]" class="form-control" id="description" placeholder="Короткий опис" value="<?= h($category[$lang['id']]['description']) ?>">
                                </div>

                                <div class="form-group">
                                    <label for="keywords">Ключові слова</label>
                                    <input type="text" name="category_desc[<?= $lang['id'] ?>][keywords]" class="form-control" id="keywords" placeholder="Ключові слова" value="<?= h($category[$lang['id']]['keywords']) ?>">
                                </div>

                                <div class="form-group">
                                    <label for="content">Опис категорії</label>
                                    <textarea name="category_desc[<?= $lang['id'] ?>][content]" class="form-control editor" id="content" rows="3" placeholder="Опис категорії"><?= h($category[$lang['id']]['content']) ?></textarea>
                                </div>
